...
!1,!1,{accept:d});a=e.find("input[type=file]");a.attr("title",XF.htmlspecialchars(XF.phrase("attach")));a.css("overflow","hidden");a.css(XF.isRtl()?"right":"left",-1E3)})},fileAdded:function(a){var b=this.applyUploadTemplate({filename:a.name,uploading:!0});this.resizeProgress(b,0);b.data("file",a);this.$filesContainer.addClass("is-active");b.appendTo(this.$filesContainer);this.fileMap[a.uniqueIdentifier]=b;this.$target.find(this.options.uploadButton).blur();if(0<XF.config.uploadMaxFilesize&&a.size>
XF.config.uploadMaxFilesize)return this.uploadError(a,this.addErrorToJson({},XF.phrase("uploaded_file_is_too_large_for_server_to_process"))),!1},uploadProgress:function(a){var b=this.fileMap[a.uniqueIdentifier];b&&(this.setUploading(!0),this.resizeProgress(b,a.progress()))},resizeProgress:function(a,b){b=Math.floor(100*b);a=a.find(this.options.templateProgress);var h=a.find("i");h.length||(h=c("<i />"),a.html("&nbsp;").append(h));h.text(b+"%").css("width",b+"%")},uploadSuccess:function(a,b,c){b=this.getObjectFromMessage(b);
this.setUploading(!1);b.status&&"error"==b.status?this.uploadError(a,b,c):b.attachment?this.insertUploadedRow(b.attachment,this.fileMap[a.uniqueIdentifier]):(b=this.addErrorToJson(b),this.uploadError(a,b,c))},setUploading:function(a){a=a?!0:!1;a!==this.isUploading&&((this.isUploading=a)?this.$target.trigger("attachment-manager:upload-start"):this.$target.trigger("attachment-manager:upload-end"))},getObjectFromMessage:function(a){if(a instanceof Object)return a;try{return c.parseJSON(a)}catch(b){return this.addErrorToJson({})}},
addErrorToJson:function(a,b){a.status="error";a.errors=[null===b?XF.phrase("oops_we_ran_into_some_problems"):b];return a},insertUploadedRow:function(a,b){a=this.applyUploadTemplate(a);this.editor||this.removeInsertButtons(a);b?b.replaceWith(a):(this.$filesContainer.addClass("is-active"),a.appendTo(this.$filesContainer));XF.activate(a);XF.layoutChange();b=c.Event("attachment:row-inserted");a.trigger(b,[a,this]);this.toggleInsertAllRow()},uploadError:function(a,b,c){b=this.getObjectFromMessage(b);this.setUploading(!1);
var d=this.fileMap[a.uniqueIdentifier];d&&b.errors?(d.find(this.options.templateProgress).remove(),d.find(this.options.templateError).text(b.errors[0]),d.addClass("is-uploadError"),delete this.fileMap[a.uniqueIdentifier],d.removeData("file")):(XF.defaultAjaxSuccessError(b,200,c.xhr),this.removeFileRow(d))},actionButtonClick:function(a){a.preventDefault();var b=c(a.currentTarget);a=b.attr("data-action");b=b.closest(this.options.fileRow);switch(a){case "thumbnail":case "full":this.insertAttachment(b,
a);break;case "delete":this.deleteAttachment(b);break;case "cancel":this.cancelUpload(b)}},allActionButtonClick:function(a){a.preventDefault();var b=c(a.currentTarget).attr("data-action");a=this.$filesContainer.find(this.options.fileRow+" "+this.options.actionButton);a=a.filter(function(){return c(this).attr("data-action")===b});a.click()},insertAttachment:function(a,b){var c=a.data("attachment-id");if(c&&this.editor){var d=a.find(this.options.templateThumb).attr("src");a=a.find(this.options.templateView).attr("href");
var e={id:c,img:d};if("full"==b)c="[ATTACH=full]"+c+"[/ATTACH]",b='<img src="{{img}}" data-attachment="full:{{id}}" alt="{{id}}" />',e.img=a;else{if(!d)return;c="[ATTACH]"+c+"[/ATTACH]";b='<img src="{{img}}" data-attachment="thumb:{{id}}" alt="{{id}}" />'}b=Mustache.render(b,e);XF.insertIntoEditor(this.$target,b,c,"[data-attachment-target=false]")}},deleteAttachment:function(a){var b=a.data("attachment-id");if(b){var e=this;XF.ajax("post",this.manageUrl,{delete:b},function(b){b.delete&&e.removeFileRow(a)},
{skipDefaultSuccess:!0});var d=new RegExp("^[a-z]+:"+b+"$","i"),f=new RegExp("\\[attach(=[^\\]]*)?\\]"+b+"\\[/attach\\]","gi");XF.modifyEditorContent(this.$target,function(a){var b=a.ed;b.$el.find("img[data-attachment]").filter(function(){return d.test(c(this).attr("data-attachment"))}).each(function(){b.image.remove(c(this))})},function(a){var b=a.val();b=b.replace(f,"");a.val(b)},"[data-attachment-target=false]")}},cancelUpload:function(a){var b=a.data("file");a.data("attachment-id")||b&&1==b.progress()||
this.removeFileRow(a)},uploadQueryParams:function(){return{_xfToken:XF.config.csrf,_xfResponseType:"json",_xfWithData:1}},applyUploadTemplate:function(a){a=c(c.parseHTML(Mustache.render(this.template,a)));var b=this.options.fileRow;return a.filter(function(){return c(this).is(b)})},removeFileRow:function(a){a.remove();this.toggleInsertAllRow();this.$filesContainer.find(this.options.fileRow).length||(this.$filesContainer.removeClass("is-active"),XF.layoutChange())},removeInsertButtons:function(a){a.find(this.options.insertRow+
