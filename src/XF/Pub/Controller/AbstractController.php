...
abstract class AbstractController extends \XF\Mvc\Controller
{
	...
	public function assertPolicyAcceptance($action)
	{
		...
		{
			// custom privacy policy URL route path matches route path of current request URI so allow it through
			if (!empty($options->privacyPolicyUrl['custom']) && rtrim($request->getRoutePathFromUrl($privacyPolicyUrl), '/') == rtrim($request->getRoutePathFromUrl($requestUri), '/'))
			{
				return;
			}
		...
		{
			// custom terms URL route path matches route path of current request URI so allow it through
			if (!empty($options->tosUrl['custom']) && rtrim($request->getRoutePathFromUrl($tosUrl), '/') == rtrim($request->getRoutePathFromUrl($requestUri), '/'))
			{
				return;
			}
	...
	public function assertTermsAccepted($action)
	{
		$visitor = \XF::visitor();
		$lastUpdate = $this->options()->termsLastUpdate;

		if ($lastUpdate && $visitor->user_id)
		{
