...
class Account extends AbstractController
{
	...
	protected function preferencesSaveProcess(\XF\Entity\User $visitor)
	{
		...
		$input['option']['alert_optout'] = $optOuts;
	...
	public function actionLikes()
	{
		...
		$likeFinder = $likeRepo->findUserLikes($visitor);

		$total = $likeFinder->total();
		$this->assertValidPage($page, $perPage, $total, 'account/likes');
		...
		$view = $this->view('XF:Account\Likes', 'account_likes', $viewParams);
		return $this->addAccountWrapperParams($view, 'likes');
	}
