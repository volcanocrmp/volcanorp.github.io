...
class Conversation extends AbstractController
{
	...
	public function actionMessagesLike(ParameterBag $params)
	{
		$message = $this->assertViewableMessage($params->message_id);
		...
		return $likePlugin->actionToggleLike(
			$message,
			$this->buildLink('conversations/messages/like', $message),
			$this->buildLink('conversations/messages', $message),
			$this->buildLink('conversations/messages/likes', $message)
		);
	}

	public function actionMessagesLikes(ParameterBag $params)
	{
		$message = $this->assertViewableMessage($params->message_id);
		...
		$title = \XF::phrase('members_who_liked_message_by_x', ['user' => $message->User->username]);
