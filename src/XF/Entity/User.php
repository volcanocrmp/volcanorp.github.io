...
class User extends Entity
{
	...
	public function isPossibleSpammer(&$error = null)
	{
		...
		if ($criteria['like_count'] && $this->like_count > $criteria['like_count'])
		{
			$error = \XF::phraseDeferred('spam_cleaner_too_many_likes', ['like_count' => $criteria['like_count']]);
			return false;
		}
	...
	protected function verifyEmail(&$email)
	{
		...
		if (!$this->getOption('admin_edit'))
		{
			$emailValidator->setOption('banned', $this->app()->container('bannedEmails'));
		}
	...
	public static function getStructure(Structure $structure)
	{
		...
		$structure->columns = [
			...
			'message_count' => ['type' => self::UINT, 'forced' => true, 'default' => 0, 'changeLog' => false],
			...
			'register_date' => ['type' => self::UINT, 'default' => \XF::$time],
			'last_activity' => ['type' => self::UINT, 'default' => \XF::$time, 'changeLog' => false],
			'trophy_points' => ['type' => self::UINT, 'forced' => true, 'default' => 0, 'changeLog' => false],
			...
			'like_count' => ['type' => self::UINT, 'forced' => true, 'default' => 0, 'changeLog' => false],
