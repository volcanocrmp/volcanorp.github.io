 ...
 * @property \XF\Entity\ProfilePostComment[] Comments
 */
class ProfilePost extends Entity
{
	...
	protected function profilePostHidden($hardDelete = false)
	{
		if (!$hardDelete)
		{
			...
			/** @var \XF\Repository\LikedContent $likeRepo */
			$likeRepo = $this->repository('XF:LikedContent');
			$likeRepo->fastUpdateLikeIsCounted('profile_post_comment', $this->comment_ids, false);
		}
	...
	protected function _postDelete()
	{
		...
		if ($commentIds)
		{
			/** @var \XF\Repository\LikedContent $likeRepo */
			$likeRepo = $this->repository('XF:LikedContent');
			$likeRepo->fastDeleteLikes('profile_post_comment', $commentIds);
	...
	public static function getStructure(Structure $structure)
	{
		...
		$structure->columns = [
			...
			'latest_comment_ids' => ['type' => self::SERIALIZED_ARRAY, 'default' => []]
		];
		$structure->behaviors = [
